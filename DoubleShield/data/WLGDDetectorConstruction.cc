...
#define whichGeometry 1
  ...
  if(fWithBoratedPET == 3)
	...
	new G4LogicalVolume(boratedPETSolid_Tube, BoratedPETMat, "BoratedPET_Logical_Lid");
      ...
      //~A "hack" to implement a second, secret, smaller shield nested inside the first one
      //It will be the same thickness, but 1 m smaller in height and diameter
      //The height is implemented as "half-height" in G4, so will be reduced by 50 cm
      //Radius is half of diameter, so it also is reduced by 50 cm
      /*      boratedPETSolid_Tube =
	new G4Tubs("BoratedPET", (fBoratedTurbineRadius - 50) * cm,
		   ((fBoratedTurbineRadius - 50) * cm + b_width * 2), (b_height - 50*cm), 0.0, CLHEP::twopi);
      fBoratedPETLogical_Tube =
	new G4LogicalVolume(boratedPETSolid_Tube, BoratedPETMat, "BoratedPET_Logical");
      ...
      boratedPETSolid_Tube =
	new G4Tubs("BoratedPET", 0, (fBoratedTurbineRadius * cm + b_width * 2 - 50*cm), b_width,
		   0.0, CLHEP::twopi);
      fBoratedPETLogical_Tube =
	new G4LogicalVolume(boratedPETSolid_Tube, BoratedPETMat, "BoratedPET_Logical_Lid");

      new G4PVPlacement(
			nullptr, G4ThreeVector(0, 0, fBoratedTurbinezPosition * cm - b_height - b_width + 50*cm),
			fBoratedPETLogical_Tube, "BoratedPET_phys", fLarLogical, false, 0, true);
      new G4PVPlacement(
			nullptr, G4ThreeVector(0, 0, fBoratedTurbinezPosition * cm + b_height + b_width - 50*cm),
			fBoratedPETLogical_Tube, "BoratedPET_phys", fLarLogical, false, 0, true);
