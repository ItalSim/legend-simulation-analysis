...
void WLGDSteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  if(aStep->GetTrack()->GetDefinition()->GetPDGEncoding()==2112)//Is a neutron
    {
      //Fill the variables for the step-level ntuple
      NeutronStepX    = aStep->GetPostStepPoint()->GetPosition().x()/CLHEP::mm;
      NeutronStepY    = aStep->GetPostStepPoint()->GetPosition().y()/CLHEP::mm;
      NeutronStepZ    = aStep->GetPostStepPoint()->GetPosition().z()/CLHEP::mm;
      NeutronStepTime = aStep->GetPostStepPoint()->GetGlobalTime()/CLHEP::ns;
      NeutronStepKineticEnergy = aStep->GetPostStepPoint()->GetKineticEnergy()/CLHEP::keV;
      NeutronStepTrackID       = aStep->GetTrack()->GetTrackID();
      NeutronStepID            = aStep->GetTrack()->GetCurrentStepNumber();
      NeutronStepProcess       = aStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName();
      NeutronStepMaterial      = aStep->GetPreStepPoint()->GetMaterial()->GetName();
      NeutronStepVolume        = aStep->GetPreStepPoint()->GetPhysicalVolume()->GetName();
      
      if(NeutronStepMaterial!="G4_WATER"&&NeutronStepMaterial!="StdRock"&&NeutronStepMaterial!="GdLoadedWater")
	{
	  //Fill the ntuple itself
	  auto *am = G4AnalysisManager::Instance();
	  //	    G4cout << NeutronStepMaterial << G4endl;
	  am->FillNtupleDColumn(1,0,NeutronStepX);
	  am->FillNtupleDColumn(1,1,NeutronStepY);
	  am->FillNtupleDColumn(1,2,NeutronStepZ);
	  am->FillNtupleDColumn(1,3,NeutronStepTime);
	  am->FillNtupleDColumn(1,4,NeutronStepKineticEnergy);
	  am->FillNtupleIColumn(1,5,NeutronStepTrackID);
	  am->FillNtupleIColumn(1,6,NeutronStepID);
	  am->FillNtupleSColumn(1,7,NeutronStepProcess);
	  am->FillNtupleSColumn(1,8,NeutronStepMaterial);
	  am->FillNtupleSColumn(1,9,NeutronStepVolume);
	  am->AddNtupleRow(1);
	}
    }//If is neutron
